...
RR = np.diag([0.02,0.02,0.02])


roll_max_angle  = 30 *np.pi/180 #rad
pitch_max_angle = 30 *np.pi/180 #rad
yaw_max_angle   = 60 *np.pi/180#45 *np.pi/180 #rad/sec
...
Kp_yaw   = 63
beta = 1.8
c = np.array([2*beta, 0.5*beta, 2*beta, 0.5*beta, 3.5*beta, 1.5*beta, 9*beta, 3*beta, 9*beta, 3*beta, 2*beta, 2*beta])
...
ramda1  = 0.1#0.01#0.5#0.005
ramda3  = 0.1#0.01#0.5#0.005
ramda5  = 0.1#0.01#0.5#0.005
ramda7  = 0.1#0.01#0.5#0.005
ramda9  = 0.1#0.01#0.5#0.005
ramda11 = 0.1#0.01#0.5#0.005
...
acc_k2 = np.exp(2*del_t*(-np.pi*acc_hz*np.sqrt(2)))
acc_k1 = 2*np.exp((-np.pi*acc_hz*np.sqrt(2))*del_t)*np.cos(np.pi*acc_hz*np.sqrt(2)*del_t)
acc_k0 = 1-acc_k1+acc_k2

gyro_k2 = np.exp(2*del_t*(-np.pi*gyro_hz*np.sqrt(2)))
gyro_k1 = 2*np.exp((-np.pi*gyro_hz*np.sqrt(2))*del_t)*np.cos(np.pi*gyro_hz*np.sqrt(2)*del_t)
gyro_k0 = 1-gyro_k1+gyro_k2

acc_low2 = np.array([0,0,9.81])
acc_low1 = np.array([0,0,9.81])
acc_low  = np.array([0,0,9.81])
gyro_low2 = np.array([0,0,0])
gyro_low1 = np.array([0,0,0])
gyro_low  = np.array([0,0,0])
...
#########################################################################################
    ...
    acc_low = acc_k1*acc_low1 -acc_k2*acc_low2 + acc_k0*np.array([m9a[0],m9a[1],m9a[2]]) 

    acc_low2 = acc_low1
    acc_low1 = acc_low

    gyro_low = gyro_k1*gyro_low1 -gyro_k2*gyro_low2 +gyro_k0*np.array([m9g[0],m9g[1],m9g[2]])

    gyro_low2 = gyro_low1
    gyro_low1 = gyro_low

    
    acc = np.array([-acc_low[0],-acc_low[1],-acc_low[2]])

    omega = np.array([(gyro_low[0]-bias_gyro_x),(gyro_low[1]-bias_gyro_y),gyro_low[2]-bias_gyro_z])

    yy = np.array([[-acc_low[0]],
                   [-acc_low[1]],
                   [-acc_low[2]]])

    P_new = (I_9-GG.dot(CC)).dot(P_pre)
    x_new = x_pre + GG.dot(yy-h_pre)
#    print "{:+7.3f}".format(m9g[0]),",","{:+7.3f}".format(gyro_low[0]),",","{:+7.3f}".format(x_new[:,0][6]*180/np.pi)
#    print "{:+7.3f}".format(x_new[:,0][6]*180/np.pi),",","{:+7.3f}".format(x_new[:,0][7]*180/np.pi),",","{:+7.3f}".format(x_new[:,0][8]*180/np.pi)

#########################################################################################


    roll  = 0# (float(rcin.read(3))-1514) * roll_max_angle / 410
    pitch = 0#(float(rcin.read(1))-1514) * pitch_max_angle / 410#+11*np.pi/180
    yaw   = -(float(rcin.read(0))-1514) * yaw_max_angle / 410
    throt =  (float(rcin.read(2))-1104) * 14000 / 820

#    print "{:+7.3f}".format(x_new[:,0][6]*180/np.pi),",","{:+7.3f}".format(x_new[:,0][7]*180/np.pi),",","{:+7.3f}".format(x_new[:,0][8]*180/np.pi),",","{:+7.3f}".format(pitch*180/np.pi)
    ...
    if throt > 10:
        ...
        omega1 = gyro_low[0]-bias_gyro_x
        omega2 = gyro_low[1]-bias_gyro_y
        omega3 = gyro_low[2]-bias_gyro_z
        
        tau_pitch = 0#1000 * 1000 * I_x * ((1+ramda7+c[6]*c[7])*(pitch-x_new[:,0][6])+(c[6]+c[7])*(-omega1)+c[7]*ramda7*e_7-a_1*omega2*omega3)+1000*1000*0.045*0.13*9.81*np.sin(x_new[:,0][6])
        
        tau_roll = 0#1000 * 1000 * I_y * ((1+ramda9+c[8]*c[9])*(roll-x_new[:,0][7])+(c[8]+c[9])*(-omega2)+c[9]*ramda9*e_9-a_2*omega1*omega3)+1000*1000*0.04*0.13*9.81*np.sin(x_new[:,0][7])
        ...
        print "{:+7.3f}".format(x_new[:,0][8]*180/np.pi),",","{:+7.3f}".format(yaw*180/np.pi),",","{:+7.3f}".format(tau_yaw)
