...
#    print throt
    if throt > 10:
        ...
        tau_pitch = 1000 * 1000 * I_x * ((1+ramda7+c[6]*c[7])*(pitch-x_new[:,0][6])+(c[6]+c[7])*(-omega1)+c[7]*ramda7*e_7-a_1*omega2*omega3)#+1000*1000*0.01*0.13*9.81*np.sin(x_new[:,0][6])
        
        tau_roll = 1000 * 1000 * I_y * ((1+ramda9+c[8]*c[9])*(roll-x_new[:,0][7])+(c[8]+c[9])*(-omega2)+c[9]*ramda9*e_9-a_2*omega1*omega3)#+1000*1000*0.01*0.13*9.81*np.sin(x_new[:,0][7])
