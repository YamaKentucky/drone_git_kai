...
Kp_yaw   = 63
beta = 1.6
c = np.array([2*beta, 0.5*beta, 2*beta, 0.5*beta, 3.5*beta, 1.5*beta, 8*beta, 2*beta, 8*beta, 2*beta, 2*beta, 2*beta])
...
#    print "{:+7.3f}".format(x_new[:,0][6]*180/np.pi),",","{:+7.3f}".format(x_new[:,0][7]*180/np.pi),",","{:+7.3f}".format(x_new[:,0][8]*180/np.pi),",","{:+7.3f}".format(pitch*180/np.pi)
    ...
    if throt > 10:
        ...
        tau_pitch = 0#1000 * 1000 * I_x * ((1+ramda7+c[6]*c[7])*(pitch-x_new[:,0][6])+(c[6]+c[7])*(-omega1)+c[7]*ramda7*e_7-a_1*omega2*omega3)
